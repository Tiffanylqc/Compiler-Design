...
public class ParameterMemoryAllocator implements MemoryAllocator {
	...
	public MemoryLocation allocate(int sizeInBytes) {
		...
		updateMin();
		MemoryLocation newMemoryLocation=new MemoryLocation(accessor, baseAddress, currentOffset);
		memoryLocations.add(newMemoryLocation);
	...
	public void restoreState() {
		...
		if(bookmarks.size()==1){
			int maxAllocateSize=this.getMaxAllocatedSize();
			for(MemoryLocation memoryLocation:memoryLocations){
				int oldOffset=memoryLocation.getOffset();
				memoryLocation.setOffset(oldOffset+maxAllocateSize);
			}
		}
